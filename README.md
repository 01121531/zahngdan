...
安装后，登录“轻账”的设置页即可检查并安装 GitHub `main` 分支上的新版本。更新器使用随机令牌验证应用请求，并在切换前运行 lint、类型检查、测试和生产构建；新版本健康检查失败时会自动恢复上一版本。账单数据库与附件保存在 `/var/lib/qingzhang`，不随程序版本切换。若 Worker 无法访问默认回环地址，可通过 `QINGZHANG_UPDATE_HOST_OVERRIDE` 指定服务器自身地址，并确保防火墙不向外开放更新端口 `8871`。
