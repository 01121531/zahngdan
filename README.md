...
- 收入、支出记录与月度统计
- 分类、支付方式和多条件筛选，支持按月、全部时间和自定义日期区间
...
安装后，登录“轻账”的设置页即可检查并安装 GitHub `main` 分支上的新版本。网页将更新请求写入本机数据库队列，更新器使用短生命周期只读连接检查任务，只在更新开始和结束时短暂写入状态；专用 `qingzhang` 账户只能调用固定的部署脚本，不开放额外网络端口。切换前会运行 lint、类型检查、测试和生产构建；新版本健康检查失败时会自动恢复上一版本。账单数据库与附件保存在 `/var/lib/qingzhang`，不随程序版本切换。
