...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const count = await getD1().prepare('SELECT COUNT(*) AS count FROM attachments WHERE transaction_id = ? AND deleted_at IS NULL').bind(transactionId).first<{ count: number }>(); if ((count?.count || 0) >= MAX_ATTACHMENTS) return jsonError('每笔账单最多10个附件');
