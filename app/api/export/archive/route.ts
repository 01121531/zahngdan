import { Zip, ZipDeflate, ZipPassThrough, strToU8 } from 'fflate'; import { requireAuth } from '@/lib/auth'; import { ensureDatabase, getD1, getFiles } from '@/lib/database'; import { exportTransactions, transactionsCsv } from '@/lib/export-data'; import { safeDownloadName } from '@/lib/files';
export async function GET(request: Request) {
  const unauthorized = await requireAuth(request); if (unauthorized) return unauthorized; await ensureDatabase(); const rows = await exportTransactions(new URL(request.url)); const ids = new Set(rows.map((row) => row.id));
