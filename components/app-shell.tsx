...
export function AppShell({ title, eyebrow, children, actions }: { title: string; eyebrow?: string; children: React.ReactNode; actions?: React.ReactNode }) {
  ...
  return <div className="min-h-dvh bg-[var(--page)] text-[var(--ink)]">
    ...
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-[232px] border-r border-[var(--line)] bg-[var(--surface)] px-5 py-7 lg:flex lg:flex-col">
      <Link href="/" className="flex items-center gap-3 px-2"><span className="grid size-10 place-items-center rounded-[14px] bg-[var(--brand)] text-sm font-bold text-white shadow-[0_8px_24px_rgba(15,118,110,.22)]">轻</span><span><strong className="block text-[17px] tracking-tight">轻账</strong><small className="text-xs text-[var(--muted)]">每一笔，都清楚</small></span></Link>
      <nav aria-label="主导航" className="mt-10 space-y-2">{primary.map(({ href, label, icon: Icon }) => <Link key={href} href={href} aria-current={isActive(href) ? 'page' : undefined} className={cn('flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-medium', isActive(href) ? 'bg-[var(--brand-soft)] text-[var(--brand-strong)]' : 'text-[var(--muted)] hover:bg-[var(--hover)] hover:text-[var(--ink)]')}><Icon size={19} strokeWidth={1.8} />{label}</Link>)}</nav>
      ...
      <nav aria-label="次要导航" className="mt-3 space-y-1">{secondary.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={cn('flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-medium text-[var(--muted)] hover:bg-[var(--hover)] hover:text-[var(--ink)]', isActive(href) && 'bg-[var(--hover)] text-[var(--ink)]')}><Icon size={18}/>{label}</Link>)}</nav>
    </aside>
    <main id="main-content" className="mx-auto min-h-dvh max-w-[1480px] px-4 pb-28 pt-5 sm:px-7 lg:ml-[232px] lg:px-10 lg:pb-12 lg:pt-8">
      <header className="flex items-center justify-between gap-4"><div><p className="text-sm font-medium text-[var(--muted)]">{eyebrow || new Intl.DateTimeFormat('zh-CN', { dateStyle: 'full' }).format(new Date())}</p><h1 className="mt-1 text-[28px] font-bold tracking-[-0.035em] sm:text-[32px]">{title}</h1></div><div className="flex shrink-0 items-center gap-2">{actions || <Link href="/transactions/new" className={buttonClass()}><Plus size={18}/>记一笔</Link>}</div></header>
      <div className="mt-7">{children}</div>
    </main>
    <nav aria-label="移动端导航" className="fixed inset-x-3 bottom-3 z-30 grid grid-cols-4 rounded-[20px] border border-[var(--line)] bg-[var(--surface-glass)] p-2 shadow-[0_18px_50px_rgba(16,24,22,.16)] backdrop-blur-xl lg:hidden">{[...primary, { href: '/settings', label: '设置', icon: Settings }].map(({ href, label, icon: Icon }) => <Link key={href} href={href} aria-current={isActive(href) ? 'page' : undefined} className={cn('flex min-h-12 flex-col items-center justify-center gap-1 rounded-[14px] text-[11px] font-medium', isActive(href) ? 'bg-[var(--brand-soft)] text-[var(--brand-strong)]' : 'text-[var(--muted)]')}><Icon size={19} strokeWidth={1.8}/>{label}</Link>)}</nav>
  </div>;
}

export function InlineLink({ href, children }: { href: string; children: React.ReactNode }) { return <Link href={href} className="inline-flex min-h-10 items-center gap-1 text-sm font-semibold text-[var(--brand)] hover:text-[var(--brand-strong)]">{children}<ChevronRight size={16}/></Link>; }
