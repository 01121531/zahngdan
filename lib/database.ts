...
async function initializeDatabase() {
  ...
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS transaction_requests (request_id TEXT PRIMARY KEY NOT NULL, transaction_id TEXT NOT NULL REFERENCES transactions(id) ON DELETE CASCADE, created_at TEXT NOT NULL)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_transaction_requests_transaction ON transaction_requests(transaction_id)`,
...
export function ensureDatabase() {
  if (!databaseReady) databaseReady = initializeDatabase().catch((error) => { databaseReady = null; throw error; });
...
export async function retryDatabase<T>(operation: () => Promise<T>, attempts = 3): Promise<T> {
  let lastError: unknown;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try { return await operation(); } catch (error) {
      if (!isDatabaseBusy(error) || attempt === attempts - 1) throw error;
      lastError = error;
